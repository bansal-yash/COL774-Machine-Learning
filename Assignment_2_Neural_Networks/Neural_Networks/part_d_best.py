import numpy as np
import os
import part_d_trainloader as trainload
import part_d_testloader as testload
import pickle
import argparse
...
# python3 part_d_best.py --dataset_root dataset/multi_dataset --test_dataset_root dataset/multi_dataset --save_weights_path weights.pkl --save_predictions_path predictions.pkl
...
test_root = args.test_dataset_root
weights_file = args.save_weights_path
predictions_file = args.save_predictions_path
train_csv = os.path.join(train_root, "train.csv")
test_csv = os.path.join(test_root, "val.csv")
...
train_dataset = trainload.TrainImageDataset(
    train_root,
    train_csv,
    trainload.numpy_transform,
)
train_dataloader = trainload.TrainDataLoader(train_dataset, batch_size=batch_size)

test_dataset = testload.TestImageDataset(
    test_root,
    test_csv,
    testload.numpy_transform,
)
test_dataloader = testload.TestDataLoader(test_dataset, batch_size=4000)
...
for x_batch, label_batch in train_dataloader:
...
for curr_epoch in range(1, num_epochs + 1):
    ...
    if temp_loss <= curr_best_loss:
        ...
        test_layer_activations = [x_test]
        for i in range(1, num_layers):
            fc = (
                np.dot(test_layer_activations[i - 1], curr_best_weight_list[i - 1])
                + curr_best_biases_list[i - 1]
            )
            fc = 1 / (1 + np.exp(-fc))
            test_layer_activations.append(fc)

        out_layer = (
            np.dot(
                test_layer_activations[num_layers - 1],
                curr_best_weight_list[num_layers - 1],
            )
            + curr_best_biases_list[num_layers - 1]
        )
        predictions = np.argmax(out_layer, axis=1)
